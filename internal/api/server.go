...
func makeHealthzHandler(db *store.SQLite) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		jsonOK(c, map[string]interface{}{
			...
			"signal_group_commit_batches_total":      pipeline.SignalBatchCount,
			"signal_group_commit_events_total":       pipeline.SignalBatchEvents,
			"last_signal_group_commit_size":          pipeline.LastSignalBatchSize,
			"max_signal_group_commit_size":           pipeline.MaxSignalBatchSize,
			"last_signal_group_commit_ms":            pipeline.LastSignalCommitMillis,
			"max_signal_group_commit_ms":             pipeline.MaxSignalCommitMillis,
			"last_upstream_head_number":              pipeline.LastHeadNumber,
			"last_upstream_head_timestamp_ms":        pipeline.LastHeadTimestampMillis,
			"last_upstream_head_received_at_ms":      pipeline.LastHeadReceivedMillis,
			"last_upstream_head_delay_ms":            pipeline.LastHeadDelayMillis,
			"max_upstream_head_delay_ms":             pipeline.MaxHeadDelayMillis,
			"last_upstream_head_arrival_gap_ms":      pipeline.LastHeadArrivalGapMillis,
			"last_upstream_head_block_gap":           pipeline.LastHeadBlockGap,
			"upstream_head_delay_over_1s_total":      pipeline.HeadDelayOverOneSecond,
			"upstream_head_catchup_total":            pipeline.HeadCatchupBursts,
			"upstream_head_clock_adjusted_total":     pipeline.HeadClockAdjusted,
			"last_upstream_log_after_head_ms":        pipeline.LastLogAfterHeadMillis,
			"max_upstream_log_after_head_ms":         pipeline.MaxLogAfterHeadMillis,
			"upstream_log_after_head_over_1s_total":  pipeline.LogAfterHeadOverOneSecond,
			"upstream_log_before_head_total":         pipeline.LogObservedBeforeHead,
