...
.charthead{display:flex;align-items:center;justify-content:space-between;gap:12px}.charthead h2{margin:0}.charthead select{background:#0d1421;color:var(--text);border:1px solid var(--line);border-radius:7px;padding:7px 10px}.chartlegend{display:flex;gap:18px;color:var(--muted);font-size:12px;margin:12px 0}.chartlegend span:before{content:'';display:inline-block;width:18px;height:3px;margin-right:6px;vertical-align:middle;border-radius:2px;background:var(--c)}.chartwrap{height:280px;position:relative}.chartwrap svg{width:100%;height:100%;overflow:visible}.gridline{stroke:#273247;stroke-width:1}.axistext{fill:#8b98ad;font-size:11px}.chartline{fill:none;stroke-width:2;stroke-linejoin:round;stroke-linecap:round}.chartnote{color:var(--muted);font-size:12px;margin-top:8px}
...
<section class="panel events"><div class="charthead"><h2>Realtime 延迟趋势（最近 2 小时，5 分钟分桶）</h2><select id="metric"><option value="service">服务处理</option><option value="freshness">区块新鲜度</option><option value="network">网络传输</option></select></div><div class="chartlegend"><span style="--c:#37d67a">P50</span><span style="--c:#f5b942">P95</span><span style="--c:#ff647c">P99</span></div><div class="chartwrap" id="chart"></div><div class="chartnote">仅统计 proposed realtime；backfill 不进入曲线，空时间段不连线。</div></section>
...
let latestSeries=[];
function drawChart(){let metric=document.getElementById('metric').value,rows=latestSeries.map(x=>({...x,...x[metric]})),vals=rows.flatMap(x=>[x.p50,x.p95,x.p99]).filter(Number.isFinite),el=document.getElementById('chart');if(!vals.length){el.innerHTML='<div class="empty">当前时间窗口暂无 realtime 数据</div>';return}let W=1200,H=280,L=58,R=15,T=10,B=30,max=Math.max(...vals,1),nice=max>10000?Math.ceil(max/5000)*5000:max>1000?Math.ceil(max/1000)*1000:max>100?Math.ceil(max/100)*100:Math.ceil(max/10)*10,x=i=>L+i*(W-L-R)/Math.max(1,rows.length-1),y=v=>T+(nice-v)*(H-T-B)/nice,path=k=>{let parts=[],open=false;rows.forEach((r,i)=>{let v=r[k];if(Number.isFinite(v)){parts.push(`${open?'L':'M'}${x(i).toFixed(1)},${y(v).toFixed(1)}`);open=true}else open=false});return parts.join(' ')};let grid=[0,.25,.5,.75,1].map(f=>{let yy=T+f*(H-T-B),v=nice*(1-f);return `<line class="gridline" x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text class="axistext" x="${L-7}" y="${yy+4}" text-anchor="end">${fmt(v)}</text>`}).join(''),ticks=rows.map((r,i)=>i%Math.max(1,Math.floor(rows.length/6))==0?`<text class="axistext" x="${x(i)}" y="${H-7}" text-anchor="middle">${r.time}</text>`:'').join('');el.innerHTML=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">${grid}<path class="chartline" stroke="#37d67a" d="${path('p50')}"/><path class="chartline" stroke="#f5b942" d="${path('p95')}"/><path class="chartline" stroke="#ff647c" d="${path('p99')}"/>${ticks}</svg>`}
